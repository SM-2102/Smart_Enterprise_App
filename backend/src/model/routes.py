...
@model_router.post(
    "/cost_details", status_code=status.HTTP_200_OK, response_model=CostDetails
)
async def cost_details(
    ...
    cost_details = await model_service.get_cost_details(
        session, data.division, data.model
    )
    return cost_details
